...
class Trans2Seg(SegBaseModel):
    ...
    def __init__(self):
        ...
        vit_params_32 = {'embed_dim':32, 'depth': 1, 'num_heads': 4, 'mlp_ratio': 2.0, 'hid_dim': 32, 'decoder_feat_HxW': 1024}
        vit_params_16 = {'embed_dim':16, 'depth': 1, 'num_heads': 4, 'mlp_ratio': 2.0, 'hid_dim': 32, 'decoder_feat_HxW': 1024}
        vit_params_8 = {'embed_dim':8, 'depth': 1, 'num_heads': 4, 'mlp_ratio': 2.0, 'hid_dim': 32, 'decoder_feat_HxW': 1024}
        # 编码维度=c4尺寸 1024
        ...
        self.transformer_head_32 = TransformerHead(vit_params_32, c1_channels=c1_channels, c4_channels=c4_channels, hid_dim=hid_dim)
        self.transformer_head_16 = TransformerHead(vit_params_16, c1_channels=c1_channels, c4_channels=c4_channels, hid_dim=hid_dim)
        self.transformer_head_8 = TransformerHead(vit_params_8, c1_channels=c1_channels, c4_channels=c4_channels, hid_dim=hid_dim)
        self.conv1 = torch.nn.Conv2d(in_channels = 36,out_channels = 12, kernel_size = 1)
        # self.conv2 = torch.nn.Conv2d(in_channels = 12,out_channels = 4, kernel_size = 1)
    ...
    def forward(self, x):
        ...
        x_32 = self.transformer_head_32(c4, c1)
        x_32 = F.interpolate(x_32, size, mode='bilinear', align_corners=True)
        x_16 = self.transformer_head_16(c4, c1)
        x_16 = F.interpolate(x_16, size, mode='bilinear', align_corners=True)
        x_8 = self.transformer_head_8(c4, c1)
        x_8 = F.interpolate(x_8, size, mode='bilinear', align_corners=True)

        x = torch.cat((x_32, x_16, x_8), 1)
        # print(x_32.shape)
        # print(x_16.shape)
        # print(x_8.shape)
        # print(x.shape)
        ...
        return tuple(outputs), x_32, x_16, x_8
